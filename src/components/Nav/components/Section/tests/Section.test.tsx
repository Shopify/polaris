...
describe('<Nav.Section />', () => {
  ...
  it('passes the props to Nav.Item', () => {
    const section = mountWithAppProvider(
      <Section
        title="test"
        items={[
    ...
    expect(section.find(Nav.Item).props()).toMatchObject({
      url: '/admin',
      disabled: false,
      label: 'some label',
    });
  ...
  it('calls onClick callback when onClick on Nav.Item is triggered', () => {
    ...
    trigger(section.find(Nav.Item), 'onClick');
    animationFrame.runFrame();
