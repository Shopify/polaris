...
### Lock position of Scrollable container

Pass `true` to the `lock` prop to "freeze" the scroll position of that container. This can be useful when you have a fullscreen container and need to lock its position when some overlay content is rendered.
...
class ScrollableExample extends React.Component {
  ...
  handleScrollLock = () => {
    this.setState((state) => ({scrollLocked: !state.scrollLocked}));
  };

  render() {
    const {scrollLocked} = this.state;

    const scrollLockAction = {
      id: 'actionLockScrolling',
      content: 'Lock scrolling',
      onAction: this.handleScrollLock,
    };

    return (
      <Card title="Terms of service" actions={[scrollLockAction]} sectioned>
        <Scrollable shadow lock={scrollLocked} style={{height: '200px'}}>
          <TextContainer>
            <p>
