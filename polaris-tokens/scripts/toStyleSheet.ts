 ...
 * Note: These values don't vary by color-scheme.
 */
export function getStaticCustomPropertiesNext(metadata: MetadataBase) {
  return Object.entries(metadata)
    .map(([_, tokenGroup]) =>
      getCustomProperties(
        Object.fromEntries(
          Object.entries(tokenGroup)
            // Only include tokens with `valueNext` prop
            .filter(([_, metadataProperties]) =>
              Boolean(metadataProperties.valueNext),
            )
            // Move `valueNext` to `value` position
            .map(([tokenName, metadataProperties]) => [
              tokenName,
              {value: metadataProperties.valueNext!},
            ]),
        ),
      ),
    )
...
export async function toStyleSheet(metadata: Metadata) {
  ...
  :where(html.Polaris-Summer-Editions-2023){${getStaticCustomPropertiesNext(
    metadata,
  )}}
