...
(global as any).MutationObserver =
  ...
  (function(undefined) {
    ...
    MutationObserver.prototype = {
      ...
      observe($target, config) {
        ...
        const settings: any = {
          ...
          kids: Boolean(config.childList),
          descendents: Boolean(config.subtree),
          charData: Boolean(
            config.characterData || config.characterDataOldValue,
          ),
        };
    ...
    function createMutationSearcher($target, config) {
      /** type {Elestuct} */
      let $oldstate: any = clone($target, config); // create the cloned datastructure
      ...
      return function(mutations) {
        ...
        if (
          ...
          mutations.push(
            new (MutationRecord as MutationRecord & any)({
              type: 'characterData',
              target: $target,
    ...
    let hasAttributeBug: HTMLElement | boolean = document.createElement('i');
    hasAttributeBug.style.top = (0 as unknown) as string;
    hasAttributeBug = (hasAttributeBug.attributes as any).style.value != 'null';
    ...
    const getAttributeValue = hasAttributeBug
      ? getAttributeSimple
    ...
    function searchSubtree(mutations, $target, $oldstate, config) {
      ...
      function resolveConflicts(
        ...
        const distance = conflicts.length - 1;
    ...
    let counter = 1; // don't use 0 as id (falsy)
    /** @const */
    const expando = 'mo_id';
