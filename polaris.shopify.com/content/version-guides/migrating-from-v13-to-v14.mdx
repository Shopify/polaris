---
title: Migrating from v13 to v14
description: Polaris v14.0.0 prop replacement, removal of components, renamed components, and token changes.
navTitle: v14
icon: ColorsMajor
collapsibleTOC: true
order: 1
...
Upgrading to Polaris v14 from v13 requires several automated and manual migrations of token, component, and component prop names that have been removed, replaced, or renamed. The bulk of migrations are automated using the [@shopify/polaris-migrator](/tools/polaris-migrator) CLI tool, with the edge cases handled by find and replace in your code editor using provided RegExp searches. You can reference the [recommended migration workflow](#migration-workflow) or [glossary](#glossary) sections for additional migration support.

Not on v13 yet? You'll need to follow the [Migrating from v12 to v13 guide](/version-guides/migrating-from-v12-to-v13).

<Code
  code={{
    title: 'Upgrade to v14',
    className: 'language-bash',
    code: `npm install @shopify/polaris@14\n# or\nyarn add @shopify/polaris@14`,
  }}
/>

<CollapsibleDetails summary="Note: If you've installed other Polaris packages independently, you will also need to upgrade those to the major versions we released along with v14.">
...
git commit -m "[Automated] Migrate X from Polaris v13 to v14"
```
...
git commit -m "[Manual] Migrate X from Polaris v12 to v14"
```
...
The `AppProvider` `theme` prop no longer accepts the theme name `light-mobile`, please use the default `light` theme instead.
...
<Code
  code={{
    title: 'polaris-migrator codemod',
    className: 'language-bash',
    code: String.raw`npx @shopify/polaris-migrator v14-styles-replace-custom-property-font "**/*.{css,scss}"`,
