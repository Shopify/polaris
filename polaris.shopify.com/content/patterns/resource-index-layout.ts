...
const pattern: SingleVariantPattern = {
  ...
  | | |
  |-|-|
  |**Overview and manage resources**|Resource objects, such as products, orders and customers, are at the heart of merchants’ businesses. While resource types can be very different, they typically share many general activities, such as adding, finding, or taking action. Use the resource index layout pattern when merchants need to organize objects and carry out such activities. An example can be found in Products.|
  :caption[**Use when merchants need to:**]{side=top}`,
  usefulToKnow: `
  | | |
  |-|-|
  |Use the resource type as page title.|![](/images/patterns/app-settings-usage-1.png)|
  |Always use the primary action in the top right corner for resource creation. Remove the button if there is no such functionality.|![](/images/patterns/app-settings-usage-2.png)|
  |Set the page width to normal if the index doesn’t need full width.|![](/images/patterns/app-settings-usage-2.png)|`,
