...
examples:
  - fileName: inline-grid-with-varying-gap.tsx
    title: Gap
    description: >-
      Use the `gap` prop to set the amount of space between columns. The `gap` prop supports responsive spacing with the [Breakpoints tokens](/tokens/breakpoints).
  ...
  - fileName: inline-grid-with-set-number.tsx
    ...
    description: >-
      Control the number of columns using the `columns` prop. Column numbers can be responsively set using the [Breakpoints tokens](/tokens/breakpoints).
...
- To lay out a set of smaller components horizontally, [use the InlineStack component](/components/layout-and-structure/inline-stack)
