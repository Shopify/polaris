...
- Be independent sentences. To support [internationalization](/foundations/internationalization), they should not act as the first part of a sentence that is finished by the component’s options.
...
- To let merchants select one option from a list with less than 4 options, use [the choice list component](/components/choice-list)
- To create a select where merchants can make multiple selections, or to allow advanced formatting of option text, use an [option list](/components/option-list) inside a [popover](/components/overlays/popover)
