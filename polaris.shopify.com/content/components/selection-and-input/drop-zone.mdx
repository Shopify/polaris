...
To prevent the drag-and-drop region to be keyboard focusable in case DropZone `children` is modal activator itself, use the `restrictKeyboardAccess` prop along with custom callback `onClick`.
