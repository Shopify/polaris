...
To prevent the drag-and-drop region to be keyboard focusable in case DropZone `children` is modal activator itself, use the `restrictKeyboardAccess` prop alonf with custom callback `onClick`.
