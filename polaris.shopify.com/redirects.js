...
//
// These used to be served by Next.js' `redirects()` config. The site is now a
// static export (GitHub Pages), where that config does nothing, so
// `scripts/post-export.ts` turns every entry below into a small HTML
// page that does a meta-refresh to the destination. They are still wired into
// `next.config.js` for `next dev`, where real redirects are available.
//
// A `:slug` segment in a source is expanded at export time against the pages
// that actually exist under the destination prefix.
