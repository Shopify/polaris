...
const contentsComponentsDir = path.join(__dirname, "../content/components");
...
for (let i = 0; i < componentDirectories.length; i++) {
  ...
  if (isDir) {
    ...
    if (readmeExists) {
      ...
      const contentsComponentsDirectoryPath = path.join(
        contentsComponentsDir,
        componentDirectoryName
      );
      const contentsComponentsFilePath = path.join(contentsComponentsDirectoryPath, 'README.md');

      if (!fs.existsSync(contentsComponentsDirectoryPath)) {
        fs.mkdirSync(contentsComponentsDirectoryPath);
      }

      fs.writeFileSync(contentsComponentsFilePath, readmeSansExamples);
      ...
      visit(outTree, (node, index, parent) => {
        ...
        Object.entries(meta).forEach(([title, value], i) => {
          ...
          const examplePath = path.join(
            contentsComponentsDirectoryPath,
            exampleFileName
          );
  ...
  if (!code.includes("import ")) {
    ...
    if (Object.keys(polarisImports).length > 0) {
      importStatements.push(
        `import { ${Object.keys(polarisImports).join(
          ", "
