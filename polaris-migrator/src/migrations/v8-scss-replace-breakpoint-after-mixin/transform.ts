...
const plugin = (): Plugin => {
  return {
    postcssPlugin: 'v8-scss-replace-breakpoint-after-mixin',
    AtRule(atRule) {
      if (atRule.name === 'include') {
        const normalizedParams = atRule.params.split('\n').map(line => line.trim()).join('');

        const match = /legacy-polaris-v8.breakpoint-after\((?<params>.*)\)/.exec(normalizedParams);

        if (match && match.groups) {
          const [breakpoint, inclusive] = postcss.list.comma(match.groups.params)

          atRule.name = 'media';
          atRule.params = `(min-width: #{legacy-polaris-v8.em(${breakpoint})${inclusive === 'false' ? ' + 0.01em' : ''}})`
        }
      }
    },
  };
