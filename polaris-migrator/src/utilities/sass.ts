...
type ReplaceDeclReturn = void | {
  prop?: string;
  value?: string;
};

export type ReplaceDeclCallback = (decl: Declaration) => ReplaceDeclReturn;

interface ReplaceDeclCallbacks {
  [targetProp: string]: ReplaceDeclCallback;
}

export function replaceDecl(
  decl: Declaration,
  callbacks: ReplaceDeclCallbacks,
): ReplaceDeclReturn {
  const targetDecl = Object.entries(callbacks).find(([targetProp]) =>
    testAgainstStringOrRegExp(decl.prop, targetProp),
  );

  return targetDecl?.[1]?.(decl);
}

// Derived from the `matchesStringOrRegExp` function in `stylelint`:
// https://github.com/stylelint/stylelint/blob/168b71a0faf53324c04c97cecd9409b903fca57d/lib/utils/matchesStringOrRegExp.js#L56
