...
type AliasesToPropertyTypes<
  ...
  Alias extends PossibleAliasNames = ExtractArrayValues<
    PropertyToAliases,
    PossibleAliasNames
  >,
> = Simplify<{
  ...
  [AliasKey in Alias]: {
    ...
    [PropertyKey in keyof PropertyToAliases as // We're only dealing with arrays, but TS has lost that information by this
    // point, so we have to reassert it.
    PropertyToAliases[PropertyKey] extends readonly unknown[]
      ...
      : // Shouldn't make it here
        never]: // This conditional is necessary because A only _extends_ AliasConfig, so it could
