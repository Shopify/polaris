...
function PolarisSummerEditions2023Banner({
  ...
  const dismissButton = onDismiss ? (
    <Button
      plain
      icon={polarisSummerEditions2023Icon(status, withinContentContainer, true)}
      onClick={onDismiss}
      accessibilityLabel="Dismiss notification"
    />
  ) : null;
  ...
  const hasContent = children || actions;
  ...
  return withinContentContainer ? (
    ...
    <Box width="100%">
      <VerticalStack align="space-between">
        ...
        {hasContent && (
          <Box padding={smDown ? '3' : '4'} paddingBlockStart="3">
            <VerticalStack gap="2">
              {children}
              {actions}
            </VerticalStack>
...
function polarisSummerEditions2023Icon(
