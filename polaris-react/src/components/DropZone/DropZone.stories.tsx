...
export function WithImageFileUpload() {
  ...
  const uploadedFiles = files.length > 0 && (
    <Box width="100%" padding="400">
      <BlockStack gap="400">
        {files.map((file, index) => (
          <InlineStack gap="400" align="start" key={index}>
            <Thumbnail
              size="small"
              alt={file.name}
  ...
  return (
    <BlockStack gap="400" inlineAlign="stretch" align="center">
      {errorMessage}
...
export function SmallSizedWithCustomFilePicker() {
  ...
  const handleModalToggle = () => setActiveModal((activeModal) => !activeModal);
  ...
  const thumbnailStyle = {
    ...
    outlineOffset: 'var(--p-border-width-050)',
    cursor: 'pointer',
    overflow: 'hidden',
    minHeight: '0',
    minWidth: '0',
  ...
  const uploadImageModal = !files.length ? (
    <div style={{height: '500px'}}>
      <Modal
        activator={fileUploadThumbnail}
        size="small"
        open={activeModal}
