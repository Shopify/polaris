...
const nextConfig = {
  ...
  async redirects() {
    return [
      // We run a redirect to port 9000 for non prod environments as playroom
      // files aren't built to the public directory in dev mode. We redirect to
      // /preview/index.html here because Playroom's webpack is configured to
      // generate an html file for the preview page that reaches for assets in
      // the root directory via a relative path. In this case we don't care
      // about a pretty url, and want to make absolutely certain that the
      // browser is pointing to preview/index.html such that it resolves the
      // relative asset requests correctly.
      {
        source: '/playroom',
